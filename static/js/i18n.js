(function () {
    ...
    const exactMap = {
        ...
        '手机号': 'Phone',
        '短信码': 'SMS Code',
        '获取短信验证码': 'Fetch SMS verification code',
        '未能识别6位验证码，请从短信内容中手动复制': 'Could not detect a 6-digit code. Please copy from the SMS content.',
        '短信接口暂无内容': 'SMS API returned no content',
        '暂无可复制内容': 'Nothing to copy',
        '已复制': 'Copied',
        'Outlook 带短信验证码': 'Outlook with SMS verification',
        '格式：粘贴 JSON 数组或单个 JSON 对象；仅需关注 phoneNumber 与 smsCodeUrl，其余字段按导出格式保留即可': 'Format: paste a JSON array or a single JSON object. Only phoneNumber and smsCodeUrl are required for display; keep other export fields as-is.',
        '[{"email":"...","password":"...","clientId":"...","refreshToken":"...","phoneNumber":"...","smsCodeUrl":"..."}]': '[{"email":"...","password":"...","clientId":"...","refreshToken":"...","phoneNumber":"...","smsCodeUrl":"..."}]',
        '信任此邮件': 'Trust this email',
        '选择一封邮件查看详情': 'Select an email to view details',
        '无主题': 'No Subject',
        '创建': 'Create',
        '创建第一个临时邮箱': 'Create the first temp mailbox',
        '导入 CF 历史临时邮箱': 'Import CF Historical Temp Mailboxes',
        '请指定域名': 'Please specify a domain',
        '请输入要导入的邮箱': 'Please enter mailboxes to import',
        '请先选择 Cloudflare Temp Mail Provider': 'Please select Cloudflare Temp Mail provider first',
        '指定这批历史邮箱使用的域名；若下方填写完整邮箱地址，域名需与其一致。': 'Domain for this batch. If you paste full email addresses below, they must use this domain.',
        '格式：每行一个前缀（推荐）或完整邮箱；可选 `前缀----JWT` / `邮箱----JWT` 以便后续取信。': 'Format: one prefix (recommended) or full email per line. Optional `prefix----JWT` / `email----JWT` for fetching messages later.',
